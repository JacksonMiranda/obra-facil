-- ============================================================
-- Migration 05: Professional Visibility Status
--
-- Adds visibility_status, display_name, city, published_at to
-- the professionals table, then creates the view
-- professionals_public which centralises the eligibility rule.
--
-- Eligibility rule (single source of truth):
--   1. account_roles(role='professional', is_active=true) exists
--   2. professionals.visibility_status = 'active'
--   3. specialty is not empty
--   4. bio is not empty (>= 10 chars)
--   5. profiles.full_name is not empty
-- ============================================================
...
-- 2. Backfill ────────────────────────────────────────────────
-- Existing professionals whose account_role is active and whose
-- profile has the minimum required data become 'active'.
-- The rest stay 'draft'.
...
-- 3. View: professionals_public ──────────────────────────────
-- All public reads (home, search, detail) use this view.
-- It is the ONLY place the eligibility rule lives in SQL.
