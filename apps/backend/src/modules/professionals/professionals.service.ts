...
import type { ProfessionalWithProfile, SearchProfessionalsInput } from '@obrafacil/shared';

@Injectable()
export class ProfessionalsService {
  constructor(private readonly repo: ProfessionalsRepository) {}

  async search(rawInput: unknown): Promise<{ professionals: ProfessionalWithProfile[]; total: number }> {
    const input: SearchProfessionalsInput = SearchProfessionalsSchema.parse(rawInput);
