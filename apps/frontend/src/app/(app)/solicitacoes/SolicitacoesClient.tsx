    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center pt-24 px-8 text-center">
          <span className="material-symbols-outlined text-5xl text-slate-200 mb-3">assignment</span>
          <p className="text-sm font-semibold text-slate-500">
            {activeTab === 'andamento' ? 'Nenhuma solicitação em andamento' : 'Nenhuma solicitação finalizada'}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            {search ? 'Tente buscar por outro termo.' : 'Quando você contratar um profissional, a solicitação aparecerá aqui.'}
          </p>
        </div>
      ) : (
        <div className="px-4 md:px-8 mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" role="tabpanel">
          {filtered.map((work) => {
            const w = work;
                  ...
                  {Array.isArray(w.photos) && w.photos.length > 0 && (
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Link href={`/solicitacoes/${w.id}`} className="text-[10px] font-semibold text-trust">
                          Ver todas →
                        </Link>
                      </div>
